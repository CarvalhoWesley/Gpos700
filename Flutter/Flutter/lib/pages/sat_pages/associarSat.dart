...
class _PageAssociarSatState extends State<PageAssociarSat> {
  ...
  final cnpjSoftHouse = MaskedTextController(
      mask: "00.000.000/0000-00"); // CNPJ do Software House
  ...
  final codigoAtivacao = TextEditingController(
      text: GlobalValues.codAtivarSat); // Codigo de Ativacao do Sat
  ...
  void associarSat() async {
    ...
    if (commonGertec.isCodigoValido(codigoAtivacao.text)) {
      if (assinaturaAc.text.length != 0) {
        if (commonGertec.removeMaskCnpj(cnpj.text).length != 14 ||
            commonGertec.removeMaskCnpj(cnpjSoftHouse.text).length != 14) {
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              SizedBox(height: 20),
              WidgetsGertec.formField(cnpjSoftHouse, "CNPJ Software House: ",
                  textInputType: TextInputType.number),
              SizedBox(height: 20),
